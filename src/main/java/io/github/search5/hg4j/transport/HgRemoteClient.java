...
public class HgRemoteClient implements HgRemoteConnection {
    ...
    private byte[] executeGetBinary(String cmd) throws IOException {
        ...
        try {
            ...
            throw new io.github.search5.hg4j.errors.HgTransportException(fullUrl, "Malformed URL: " + fullUrl, e);
        }
        ...
        if (status == HttpURLConnection.HTTP_UNAUTHORIZED || status == HttpURLConnection.HTTP_FORBIDDEN) {
            throw new io.github.search5.hg4j.errors.HgAuthException(fullUrl, username != null ? username : "anonymous");
        }
        if (status != HttpURLConnection.HTTP_OK) {
            throw new io.github.search5.hg4j.errors.HgProtocolException(fullUrl, "Remote Mercurial server returned HTTP " + status + " for URL: " + fullUrl);
        }
        ...
        try (InputStream in = unwrapResponseStream(conn.getInputStream(), contentType);
            ...
            while ((count = in.read(buf)) != -1) {
                ...
                if (totalBytes > maxResponseBytes) {
                    throw new io.github.search5.hg4j.errors.HgProtocolException(fullUrl, "Security Guard: Remote server response size exceeds maximum allowed limit (100MB)");
                }
    ...
    private static class MercurialChunkedInputStream extends InputStream {
        ...
        public int read() throws IOException {
            ...
            if (b == -1) {
                eof = true;
                throw new io.github.search5.hg4j.errors.HgProtocolException("", "Unexpected EOF inside mercurial-0.2 chunk payload");
            }
            remaining--;
            return b;
        }
        ...
        private boolean readNextChunk() throws IOException {
            ...
            while (read < 4) {
                ...
                if (count == -1) {
                    ...
                    throw new io.github.search5.hg4j.errors.HgProtocolException("", "Unexpected EOF while reading mercurial-0.2 chunk length");
                }
                read += count;
            }
            ...
            if (len < 0) {
                throw new io.github.search5.hg4j.errors.HgProtocolException("", "Invalid negative chunk length: " + len);
            }
            remaining = len;
            return true;
        }
    ...
    private InputStream unwrapResponseStream(InputStream in, String contentType) throws IOException {
        if (contentType != null && contentType.contains("application/mercurial-0.2")) {
            ...
            while (read < compNameLen) {
                ...
                if (count == -1) {
                    throw new io.github.search5.hg4j.errors.HgProtocolException("", "Unexpected EOF while reading compression header in application/mercurial-0.2 stream");
                }
                read += count;
            }
            ...
            if ("zlib".equalsIgnoreCase(compName) || "deflate".equalsIgnoreCase(compName)) {
                ...
                throw new io.github.search5.hg4j.errors.HgProtocolException("", "Unsupported compression format in application/mercurial-0.2 framing: " + compName);
            }
    ...
    public String push(byte[] bundleBytes, List<String> heads) throws IOException {
        ...
        if (status != HttpURLConnection.HTTP_OK) {
            throw new io.github.search5.hg4j.errors.HgProtocolException(fullUrl, "Remote server returned HTTP " + status + " for unbundle: " + fullUrl);
        }
        ...
        try (InputStream in = conn.getInputStream();
            ...
            while ((count = in.read(buf)) != -1) {
                ...
                if (totalRead > 10 * 1024 * 1024) { // 10MB Limit Guard
                    throw new io.github.search5.hg4j.errors.HgProtocolException(fullUrl, "HTTP response size exceeded 10MB safety threshold under push");
                }
