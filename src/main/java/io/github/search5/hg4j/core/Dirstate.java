...
public class Dirstate {
    ...
    public void read(byte[] bytes) throws IOException {
        if (bytes == null) {
            throw new io.github.search5.hg4j.errors.HgCorruptDataException("Invalid dirstate file: content cannot be null");
        }

        if (bytes.length < 40) {
            throw new io.github.search5.hg4j.errors.HgCorruptDataException("Invalid dirstate file: must be at least 40 bytes");
        }
        ...
        while (buf.hasRemaining()) {
            if (buf.remaining() < 17) {
                throw new io.github.search5.hg4j.errors.HgCorruptDataException("Truncated dirstate entry header");
            }
            ...
            if (pathLen < 0 || buf.remaining() < pathLen) {
                throw new io.github.search5.hg4j.errors.HgCorruptDataException("Truncated dirstate entry path");
            }
    ...
    public void read(File file) throws IOException {
        ...
        if (bytes.length >= 12) {
            ...
            if ("dirstate-v2\n".equals(magicStr)) {
                ...
                if (!dataFile.exists()) {
                    throw new io.github.search5.hg4j.errors.HgCorruptDataException("Dirstate-v2 data file not found for uid: " + uid);
                }
                byte[] dataBytes = Files.readAllBytes(dataFile.toPath());
                if (dataBytes.length != dataLength) {
                    throw new io.github.search5.hg4j.errors.HgCorruptDataException("Dirstate-v2 data file length mismatch. Expected " + dataLength + " but got " + dataBytes.length);
                }
