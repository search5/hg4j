...
public class HgLocalClient implements HgRemoteConnection {
    ...
    public List<String> getHeads() throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            if (!isParent[i]) {
                heads.add(org.hg4j.core.NodeIdUtil.toHex(changelog.getIndexRecord(i).getNodeId()));
            }
        }
        return heads;
    }
    ...
    public byte[] getBundle(List<String> common, List<String> heads, List<String> bundleCaps) throws IOException {
        ...
        if (!validCommon.isEmpty()) {
            ...
            for (String c : validCommon) {
                int rev = changelog.findRevision(org.hg4j.core.NodeIdUtil.fromHex(c));
                if (rev != -1) {
        ...
        // Build bundle
        org.hg4j.core.ChangegroupParser.ChangegroupBundle bundle = new org.hg4j.core.ChangegroupParser.ChangegroupBundle();
        ...
        for (int r = startRev; r < count; r++) {
            ...
            org.hg4j.core.ChangegroupParser.ChangeGroupEntry clEntry = new org.hg4j.core.ChangegroupParser.ChangeGroupEntry();
        ...
        for (int r = startRev; r < count; r++) {
            ...
            byte[] mfNode = org.hg4j.core.NodeIdUtil.fromHex(clLines[0].trim().substring(0, 40));
            int mfRev = manifest.findRevision(mfNode);
            if (mfRev == -1) continue;

            Revlog.IndexRecord mfRec = manifest.getIndexRecord(mfRev);
            org.hg4j.core.ChangegroupParser.ChangeGroupEntry mfEntry = new org.hg4j.core.ChangegroupParser.ChangeGroupEntry();
        ...
        for (String path : affectedFiles) {
            File flIdx = org.hg4j.api.CommitCommand.getFilelogIndex(remoteRepo.getStoreDir(), path);
            File flDat = new File(flIdx.getPath().substring(0, flIdx.getPath().length() - 2) + ".d");
            if (!flIdx.exists()) continue;

            Revlog fl = remoteRepo.getRevlog(flIdx, flDat);
            List<org.hg4j.core.ChangegroupParser.ChangeGroupEntry> flEntries = new ArrayList<>();

            for (int i = 0; i < fl.getRevisionCount(); i++) {
                Revlog.IndexRecord flRec = fl.getIndexRecord(i);
                if (flRec.getLinkRev() >= startRev) {
                    org.hg4j.core.ChangegroupParser.ChangeGroupEntry flEntry = new org.hg4j.core.ChangegroupParser.ChangeGroupEntry();
            ...
            if (!flEntries.isEmpty()) {
                org.hg4j.core.ChangegroupParser.FileGroup fg = new org.hg4j.core.ChangegroupParser.FileGroup();
                fg.path = path;
    ...
    public String push(byte[] bundleBytes, List<String> heads) throws IOException {
        ...
        if (bundleBytes.length >= 6 && bundleBytes[0] == 'H' && bundleBytes[1] == 'G' && bundleBytes[2] == '1' && bundleBytes[3] == '0') {
            ...
            if ("UN".equals(comp)) {
                ...
                throw new org.hg4j.errors.HgCorruptDataException("Unsupported compression format in local push: HG10" + comp);
            }
        }

        try (java.io.ByteArrayInputStream bais = new java.io.ByteArrayInputStream(changegroupBytes)) {
            org.hg4j.core.ChangegroupParser.ChangegroupBundle bundle = 
                    org.hg4j.core.ChangegroupParser.parseBundle(bais, cgVersion);

            // Apply bundle natively to remoteRepo using transactional API of PullCommand
            org.hg4j.api.PullCommand pullApi = new org.hg4j.api.PullCommand(remoteRepo);
    ...
    private void writeEntryChunk(java.io.DataOutputStream dos, org.hg4j.core.ChangegroupParser.ChangeGroupEntry entry) throws IOException {
