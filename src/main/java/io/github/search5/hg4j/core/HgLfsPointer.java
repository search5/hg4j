...
public final class HgLfsPointer {
    ...
    public static HgLfsPointer parse(byte[] content) throws IOException {
        if (content == null || content.length == 0) {
            throw new io.github.search5.hg4j.errors.HgCorruptDataException("LFS pointer content is empty or null");
        }
        ...
        for (String line : lines) {
            ...
            if (trimmed.startsWith("version ")) {
                ...
                try {
                    ...
                    throw new io.github.search5.hg4j.errors.HgCorruptDataException("Invalid size format in LFS pointer: " + trimmed, e);
                }
            }
        }

        if (version == null || oid == null || size == -1) {
            throw new io.github.search5.hg4j.errors.HgCorruptDataException("Malformed LFS pointer file: missing required fields");
        }

        // Validate OID hex string (should be 64 characters for SHA-256)
        if (oid.length() != 64) {
            throw new io.github.search5.hg4j.errors.HgCorruptDataException("Invalid LFS OID length. Expected 64 characters, got: " + oid.length());
        }
