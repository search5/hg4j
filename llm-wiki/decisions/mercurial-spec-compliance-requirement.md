...
39. **[[exhaustive-interop-matrix-plan]] 매트릭스 범위 확장 — 명령 커버리지가
    ...
    **검증**: 전체 비-interop `test` 그린(merge 전후 각 브랜치 및 최종
    ...
    이번 wave로 로컬 명령 기준 완주 수는 31에서 **37/67**로 증가. 부수
    발견: `GraftCommand`의 v2-docket rollback/journal 기록에도 이번
    `CommitCommand` 수정과 유사한(그러나 별도인) gap이 있다는 것을
    `RecoverCommand`/`RollbackCommand` 작업 중 발견했으나 이번 wave의
    위임 범위 밖이라 손대지 않음 — `GraftCommand` 매트릭스를 맡을 다음
    에이전트가 반드시 확인할 것. 나머지 30개 로컬 명령 및 wire 매트릭스
    잔여 5개는 여전히 미착수.
