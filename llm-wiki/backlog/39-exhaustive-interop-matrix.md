...
- **후속으로 남은 것**: `GraftCommand`의 v2-docket rollback/journal gap(admin/
  maintenance 웨이브 작업 중 발견, 이 문서 완료 후 후속 처리 대상으로 명시적으로
  등록됨 — 별도 진행 상황은 커밋 이력 참고), `refreshIfChangedOnDisk()` 연결 여부의
  전수 재점검(과다 연결도 부족 연결도 둘 다 회귀를 만들 수 있음이 이번에 확인됨).
