...
| Wire protocol v1 (HTTP/SSH, capability 협상) | `hg help internals.wireprotocol` | `HgRemoteClient`, `HgSshClient`, `HgWireServer`, `TransportProtocol` | ✅ 기본 구현 존재. **라이브 서버 통신도 검증 완료(2026-09-01)** — Docker로 띄운 실제 Mercurial 6.0 `hg serve` HTTP 서버를 대상으로 hg4j `PullCommand`/`PushCommand`가 실시간 pull(수신) + push(전송 → 별도 fresh pull로 서버 반영 재확인) 왕복 성공(`HgHttpV1LiveServerInteropTest`). 실제 hg 클라이언트 → hg4j `HgWireServer` 방향(v1)은 여전히 미검증 — 백로그 참고 |
