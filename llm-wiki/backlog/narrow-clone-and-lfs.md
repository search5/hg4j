---
updated: 2026-09-06
status: items 28/30/31 completed; item 40 in progress, item 42 in progress (다른
  에이전트들이 이 문서 작성 시점에 병렬로 TDD 진행 중 — 완료되면 해당 절을 그 결과로
  갱신할 것)
---
...
wire-protocol 수준 ellipsis node 왕복 — 지금은 로컬 필터링만 있고 전송량 절감 효과가
없음, 백로그 39 완료 후 신규 등록, 진행 중, 클라이언트+서버 양방향 구현 요건), 42(LFS
세부 옵션 3가지 — rename+LFS copy-tracing, `[lfs] url` override, `lfs.disableusercache`,
백로그 31 완료 시 범위 밖으로 남겼던 것, 백로그 39 완료 후 신규 등록, 진행 중).
