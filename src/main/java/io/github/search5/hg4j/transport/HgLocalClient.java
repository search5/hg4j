...
public class HgLocalClient implements HgRemoteConnection {
    ...
    public List<String> getHeads() throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            if (!isParent[i]) {
                heads.add(io.github.search5.hg4j.core.NodeIdUtil.toHex(changelog.getIndexRecord(i).getNodeId()));
            }
        }
        return heads;
    }
    ...
    public byte[] getBundle(List<String> common, List<String> heads, List<String> bundleCaps) throws IOException {
        ...
        if (!validCommon.isEmpty()) {
            ...
            for (String c : validCommon) {
                int rev = changelog.findRevision(io.github.search5.hg4j.core.NodeIdUtil.fromHex(c));
                if (rev != -1) {
        ...
        // Build bundle
        io.github.search5.hg4j.core.ChangegroupParser.ChangegroupBundle bundle = new io.github.search5.hg4j.core.ChangegroupParser.ChangegroupBundle();
        ...
        for (int r = startRev; r < count; r++) {
            ...
            io.github.search5.hg4j.core.ChangegroupParser.ChangeGroupEntry clEntry = new io.github.search5.hg4j.core.ChangegroupParser.ChangeGroupEntry();
        ...
        for (int r = startRev; r < count; r++) {
            ...
            byte[] mfNode = io.github.search5.hg4j.core.NodeIdUtil.fromHex(clLines[0].trim().substring(0, 40));
            int mfRev = manifest.findRevision(mfNode);
            if (mfRev == -1) continue;

            Revlog.IndexRecord mfRec = manifest.getIndexRecord(mfRev);
            io.github.search5.hg4j.core.ChangegroupParser.ChangeGroupEntry mfEntry = new io.github.search5.hg4j.core.ChangegroupParser.ChangeGroupEntry();
        ...
        for (String path : affectedFiles) {
            File flIdx = io.github.search5.hg4j.api.CommitCommand.getFilelogIndex(remoteRepo.getStoreDir(), path);
            File flDat = new File(flIdx.getPath().substring(0, flIdx.getPath().length() - 2) + ".d");
            if (!flIdx.exists()) continue;

            Revlog fl = remoteRepo.getRevlog(flIdx, flDat);
            List<io.github.search5.hg4j.core.ChangegroupParser.ChangeGroupEntry> flEntries = new ArrayList<>();

            for (int i = 0; i < fl.getRevisionCount(); i++) {
                Revlog.IndexRecord flRec = fl.getIndexRecord(i);
                if (flRec.getLinkRev() >= startRev) {
                    io.github.search5.hg4j.core.ChangegroupParser.ChangeGroupEntry flEntry = new io.github.search5.hg4j.core.ChangegroupParser.ChangeGroupEntry();
            ...
            if (!flEntries.isEmpty()) {
                io.github.search5.hg4j.core.ChangegroupParser.FileGroup fg = new io.github.search5.hg4j.core.ChangegroupParser.FileGroup();
                fg.path = path;
    ...
    public String push(byte[] bundleBytes, List<String> heads) throws IOException {
        ...
        if (bundleBytes.length >= 6 && bundleBytes[0] == 'H' && bundleBytes[1] == 'G' && bundleBytes[2] == '1' && bundleBytes[3] == '0') {
            ...
            if ("UN".equals(comp)) {
                ...
                throw new io.github.search5.hg4j.errors.HgCorruptDataException("Unsupported compression format in local push: HG10" + comp);
            }
        }

        try (java.io.ByteArrayInputStream bais = new java.io.ByteArrayInputStream(changegroupBytes)) {
            io.github.search5.hg4j.core.ChangegroupParser.ChangegroupBundle bundle = 
                    io.github.search5.hg4j.core.ChangegroupParser.parseBundle(bais, cgVersion);

            // Apply bundle natively to remoteRepo using transactional API of PullCommand
            io.github.search5.hg4j.api.PullCommand pullApi = new io.github.search5.hg4j.api.PullCommand(remoteRepo);
    ...
    private void writeEntryChunk(java.io.DataOutputStream dos, io.github.search5.hg4j.core.ChangegroupParser.ChangeGroupEntry entry) throws IOException {
    ...
    public java.util.Map<String, String> listKeys(String namespace) throws IOException {
        ...
        if ("bookmarks".equals(namespace)) {
            ...
            io.github.search5.hg4j.core.PhaseRoots phaseRoots = remoteRepo.getPhaseRoots();
            File clIdx = new File(remoteRepo.getStoreDir(), "00changelog.i");
            File clDat = new File(remoteRepo.getStoreDir(), "00changelog.d");
            if (clIdx.exists()) {
                ...
                for (int i = 0; i < cl.getRevisionCount(); i++) {
                    ...
                    io.github.search5.hg4j.lib.NodeId nodeId = new io.github.search5.hg4j.lib.NodeId(nodeBytes);
                    io.github.search5.hg4j.core.PhaseRoots.Phase phase = phaseRoots.getPhase(nodeId, cl);
                    if (phase != io.github.search5.hg4j.core.PhaseRoots.Phase.PUBLIC) {
                        map.put(io.github.search5.hg4j.core.NodeIdUtil.toHex(nodeBytes), String.valueOf(phase.getValue()));
                    }
                }
            }
        }
