...
public class RemoveCommand {
    ...
    public boolean call() throws IOException {
        ...
        try (HgLock wlock = repository.lockWorkingCopy();
            ...
            if (entry == null) {
                throw new io.github.search5.hg4j.errors.HgValidationException("File is not tracked: " + file);
            }

            if (!force) {
                char state = entry.getState();
                if (state == 'a') {
                    throw new io.github.search5.hg4j.errors.HgValidationException("File has uncommitted changes (added): " + file + ". Use force to remove.");
                    ...
                    if (diskFile.exists() && diskFile.isFile()) {
                        ...
                        if (!isDirty) {
                            ...
                            if (flIdx.exists()) {
                                try {
                                    io.github.search5.hg4j.core.Revlog filelog = repository.getRevlog(flIdx, flDat);
                        ...
                        if (isDirty) {
                            throw new io.github.search5.hg4j.errors.HgValidationException("File has uncommitted changes (modified): " + file + ". Use force to remove.");
                        }
                    }
                }
            }
