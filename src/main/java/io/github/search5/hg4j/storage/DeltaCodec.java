...
public final class DeltaCodec {
     ...
     *
     * <p>{@code uncompLen} (the index record's {@code rawsize} field) is only a HINT, and for a
     * delta revision it is the WRONG size to trust as the destination buffer size -- per the
     * revlog format, that field always holds the fully-reconstructed FULLTEXT's length (what this
     * revision decodes to after the whole delta chain is applied), not the length of THIS hunk's
     * own decompressed bytes. A delta revision (real hg's own {@code mdiff}-style hunk encoding: a
     * sequence of {@code [start][end][length][data]} records describing what changed) decompresses
     * to a payload that is normally much SMALLER than the fulltext it patches. Blindly allocating
     * {@code new byte[uncompLen]} and handing back that full-sized array regardless of how many
     * bytes zstd actually wrote into it would leave trailing zero-byte padding after the real
     * delta data, which {@link DeltaEngine#applyDelta} then misparses as a bogus extra
     * {@code start=0,end=0,length=0} hunk.
     *
     * <p>Instead, the zstd frame's OWN embedded content-size header ({@link
     * Zstd#getFrameContentSize(byte[])} -- every hunk real hg writes embeds this, the same source
     * of truth {@code Revlog#decompressSidedataChunk} relies on for an analogous reason) is used
     * as the destination buffer size, falling back to {@code uncompLen} only when the frame
     * legitimately declares no content size (the zstd-jni sentinel is negative for
     * "unknown"/"error", hence the {@code >= 0} check rather than {@code > 0} -- a genuinely
     * empty-content frame is a valid, if unusual, size of zero). The returned array is always
     * trimmed down to zstd's own reported actual-bytes-written count afterward regardless of
     * which size was used to allocate it.
     *
