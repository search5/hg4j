...
public class ChangegroupParser {
    ...
    public static byte[] readChunk(InputStream in) throws IOException {
        ...
        if (payloadLen > 20 * 1024 * 1024) { // 20MB guard limit to prevent DoS OOM
            throw new io.github.search5.hg4j.errors.HgCorruptDataException("Security Guard: Changegroup chunk size exceeds maximum allowed limit (20MB): " + payloadLen);
        }
        ...
        while (offset < payloadLen) {
            ...
            if (count == -1) {
                throw new io.github.search5.hg4j.errors.HgCorruptDataException("Unexpected EOF while reading changegroup chunk payload of size: " + payloadLen);
            }
            offset += count;
        }
        return payload;
    ...
    public static List<ChangeGroupEntry> parseGroup(InputStream in, String version, String[] outVersion) throws IOException {
        ...
        while (true) {
            ...
            if (chunk.length < headerSize) {
                throw new io.github.search5.hg4j.errors.HgCorruptDataException("Malformed changegroup header chunk. Length too small: " + chunk.length + " for version: " + detectedVersion);
            }
