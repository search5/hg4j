...
public class InitCommand {
    ...
    public HgRepository call() throws IOException {
        ...
        if (!directory.exists()) {
            if (!directory.mkdirs()) {
                throw new io.github.search5.hg4j.errors.HgRepositoryNotFoundException("Failed to create repository directory: " + directory);
            }
        } else if (!directory.isDirectory()) {
            throw new io.github.search5.hg4j.errors.HgRepositoryNotFoundException("Path exists and is not a directory: " + directory);
        }

        File hgDir = new File(directory, ".hg");
        if (hgDir.exists() && !hgDir.isDirectory()) {
            throw new io.github.search5.hg4j.errors.HgRepositoryNotFoundException("Path exists and is not a directory: " + hgDir);
        }
        if (!hgDir.exists()) {
            if (!hgDir.mkdir()) {
                throw new io.github.search5.hg4j.errors.HgRepositoryNotFoundException("Failed to create .hg directory in: " + directory);
            }
        }

        File storeDir = new File(hgDir, "store");
        if (storeDir.exists() && !storeDir.isDirectory()) {
            throw new io.github.search5.hg4j.errors.HgRepositoryNotFoundException("Path exists and is not a directory: " + storeDir);
        }
        if (!storeDir.exists()) {
            if (!storeDir.mkdir()) {
                throw new io.github.search5.hg4j.errors.HgRepositoryNotFoundException("Failed to create .hg/store directory");
            }
        }
        ...
        try {
            ...
            throw new io.github.search5.hg4j.errors.HgRepositoryNotFoundException("Failed to write .hg/requires file", e);
        }
