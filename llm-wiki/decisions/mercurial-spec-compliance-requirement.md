---
updated: 2026-09-06
status: 번호 매겨진 백로그 1~44번 중 1~39번 전부 완료(25번은 오탐으로 종결). 40, 41,
  42, 43, 44번은 진행 중(별도 병렬 에이전트, 완료되면 각 backlog/*.md 파일이 갱신되고
  이 인덱스의 상태 표시도 갱신됨). 상세는 이 문서가 아니라 각 backlog/*.md 파일과
...
| Revlog inline→non-inline 성장 전환 | `_enforceinlinesize`(real hg) | `Revlog` | 🔶 진행 중(백로그 43) | [[backlog/revlog-storage-formats]] |
...
| 트랜잭션 저널링 / 크래시 복구 | `hg help internals.transaction` | `CommitCommand`, `FetchCommand`, `RollbackCommand`, `HisteditCommand` | ✅ 완료(GraftCommand v2-docket 후속 gap 남음) | [[backlog/39-exhaustive-interop-matrix]] |
...
| Clonebundles | `mercurial/wireprotov1server.py` | `ClonebundlesCommand` | ✅ 완료(서버 매니페스트-없음 응답은 백로그 44 진행 중) | [[backlog/clonebundles]] |
| Narrow clone / narrowspec | `mercurial/narrowspec.py` | `NarrowCloneCommand`, `HgTreeFilter` | ✅ 로컬 필터링 완료, wire-protocol ellipsis node는 백로그 40 진행 중 | [[backlog/narrow-clone-and-lfs]] |
| LFS(Large File Storage) | `mercurial/lfs/` | `HgLfsPointer`, `HgLfsManager` | ✅ 핵심 경로 완료, 세부 옵션 3가지는 백로그 42 진행 중 | [[backlog/narrow-clone-and-lfs]] |
...
| 40 | Narrow clone wire-protocol ellipsis node | 🔶 진행 중 | [[backlog/narrow-clone-and-lfs]] |
| 41 | SVN 서브저장소 지원 | 🔶 진행 중 | [[backlog/subrepo]] |
| 42 | LFS 세부 옵션 3가지 | 🔶 진행 중 | [[backlog/narrow-clone-and-lfs]] |
| 43 | Revlog inline→non-inline 성장 전환 | 🔶 진행 중 | [[backlog/revlog-storage-formats]] |
| 44 | Clonebundles 서버 매니페스트-없음 응답 | 🔶 진행 중 | [[backlog/clonebundles]] |
