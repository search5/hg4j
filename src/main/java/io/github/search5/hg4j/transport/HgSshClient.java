...
public class HgSshClient implements HgRemoteConnection {
    ...
    private synchronized void ensureConnected() throws IOException {
        ...
        try {
            ...
            if (e instanceof JSchException || (msg != null && (msg.toLowerCase().contains("auth fail") || msg.toLowerCase().contains("authentication") || msg.toLowerCase().contains("permission denied")))) {
                throw new io.github.search5.hg4j.errors.HgAuthException(sshUrl, username, e);
            }
            throw new io.github.search5.hg4j.errors.HgProtocolException(sshUrl, "Failed to establish SSH connection: " + msg, e);
        }
    }
    ...
    private String readLine() throws IOException {
        if (protocolVersion == 2) {
            ...
            while (read < 4) {
                ...
                if (got == -1) {
                    throw new io.github.search5.hg4j.errors.HgProtocolException(sshUrl, "Unexpected EOF while reading SSH V2 frame size");
                }
                read += got;
            }
            ...
            while (total < len) {
                ...
                if (got == -1) {
                    throw new io.github.search5.hg4j.errors.HgProtocolException(sshUrl, "Unexpected EOF while reading SSH V2 frame payload");
                }
                total += got;
            }
    ...
    private void readCapabilities() throws IOException {
        ...
        if (!header.startsWith("capabilities:")) {
            throw new io.github.search5.hg4j.errors.HgProtocolException(sshUrl, "Remote SSH server did not output valid Mercurial stdio capabilities header. Received: " + header);
        }
        ...
        if (protocolVersion == 2) {
            ...
            while (true) {
                ...
                while (read < 4) {
                    int got = in.read(lenBytes, read, 4 - read);
                    if (got == -1) {
                        throw new io.github.search5.hg4j.errors.HgProtocolException(sshUrl, "Unexpected EOF while reading SSH V2 binary frame size");
                    }
                    read += got;
                }
                int len = ((lenBytes[0] & 0xFF) << 24) |
                ...
                while (chunkRead < len) {
                    ...
                    if (got == -1) {
                        throw new io.github.search5.hg4j.errors.HgProtocolException(sshUrl, "Unexpected EOF while reading SSH V2 binary frame payload");
                    }
                    chunkRead += got;
                }
                if (channelId == 1) {
        ...
        while (true) {
            ...
            while (read < 4) {
                ...
                if (got == -1) {
                    ...
                    throw new io.github.search5.hg4j.errors.HgProtocolException(sshUrl, "Unexpected EOF while reading Mercurial SSH binary chunk size");
                }
                read += got;
            }
            int len = ((lenBytes[0] & 0xFF) << 24) |
            ...
            if (len < 0) {
                throw new io.github.search5.hg4j.errors.HgProtocolException(sshUrl, "Invalid negative binary chunk size: " + len);
            }
            ...
            while (chunkRead < len) {
                int got = in.read(chunkBuf, chunkRead, len - chunkRead);
                if (got == -1) {
                    throw new io.github.search5.hg4j.errors.HgProtocolException(sshUrl, "Unexpected EOF inside Mercurial SSH binary chunk payload");
                }
                chunkRead += got;
            }
            baos.write(chunkBuf);
