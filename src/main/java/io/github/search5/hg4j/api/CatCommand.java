...
public class CatCommand {
    ...
    public CatCommand setRevision(io.github.search5.hg4j.lib.NodeId nodeId) {
    ...
    public byte[] call() throws IOException {
        ...
        if (targetNodeId == null) {
            throw new io.github.search5.hg4j.errors.HgRevisionNotFoundException("Unable to resolve revision");
        }

        int commitRev = NodeIdUtil.findRevisionByNodeId(changelog, targetNodeId);
        if (commitRev == -1) {
            throw new io.github.search5.hg4j.errors.HgRevisionNotFoundException("Commit revision not found: " + NodeIdUtil.toHex(targetNodeId));
        }
        ...
        if (fileHexNode == null) {
            throw new io.github.search5.hg4j.errors.HgRevisionNotFoundException("File not tracked at target revision: " + file);
        }

        File flIdx = CommitCommand.getFilelogIndex(repository.getStoreDir(), file);
        File flDat = new File(flIdx.getPath().substring(0, flIdx.getPath().length() - 2) + ".d");
        if (!flIdx.exists()) {
            throw new io.github.search5.hg4j.errors.HgCorruptDataException("Filelog not found for tracked file: " + file);
        }

        Revlog filelog = repository.getRevlog(flIdx, flDat);
        int fileRev = NodeIdUtil.findRevisionByNodeId(filelog, NodeIdUtil.fromHex(fileHexNode));
        if (fileRev == -1) {
            throw new io.github.search5.hg4j.errors.HgRevisionNotFoundException("File version not found in history: " + file + " @ " + fileHexNode);
        }
